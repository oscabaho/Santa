...
namespace Santa.Presentation.Menus
{
    ...
    public class GraphicsSettingsController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                var mgr = FindFirstObjectByType<GraphicsSettingsManager>();
                if (mgr != null) _graphicsService = mgr;

                // If still null, check if we have a null service implementation or if we need to fail gracefully
                if (_graphicsService == null)
                {
                    // Try interface search
                    var found = FindFirstObjectByType<Santa.Presentation.Menus.GraphicsSettingsController>();
                    // Searching for controller won't help. 
                    // Searching for the interface via MonoBehaviour is tricky without a base class. 
                    // Assuming GraphicsSettingsManager is the only concrete implementation.
                }
            }
            ...
            if (_graphicsService == null)
            {
                ...
                // Only log error on Standalone where we expect this to work.
                // On mobile, the service might be null/stubbed and we just hide the UI.
#if UNITY_STANDALONE
